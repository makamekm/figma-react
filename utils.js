...
exports.sanitizeAttributes = function(attributes) {
  ...
  let allowed = ALLOWED_ATTRIBUTES.reduce(function(hash, name) {
    if (name in attributes) {
      let unnamespacedName = exports.unnamespaceAttributeName(name);
  ...
  let custom = Object.keys(attributes)
...
exports.styleAttribute = function(string) {
  let object = string.split(/\s*;\s*/g).reduce(function(hash, keyValue) {
    let split = keyValue.split(/\s*\:\s*/);
    let key = exports.camelCase((split[0] || '').trim());
...
exports.supportsAllAttributes = function(element) {
  let hasHyphen = element.tagName.indexOf('-') !== -1;
  let hasIsAttribute = 'is' in element.attributes;
